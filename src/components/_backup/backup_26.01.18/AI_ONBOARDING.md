...
## 🎯 최근 주요 개선사항 (2026-01-14)

### ✅ iOS Safari 무한 스와이프 뒤로가기 지원 (NEW!)
- **문제**: 홈 ↔ 콘텐츠 6회 왕복 후 또는 홈에서 연속 스와이프 뒤로가기 시 앱 종료
- **원인**: 고정 버퍼(3개)가 소진 후 재충전 로직 없음 → 히스토리 스택 고갈
- **해결**: 동적 버퍼 재충전 패턴 - `pushState`의 "현재 위치 뒤 엔트리 삭제" 특성 활용
- **핵심 원리**: 버퍼 중간 지점 이하 도달 시 `pushState`로 새 버퍼 추가 → 히스토리 길이 5~7 유지
- **핵심 파일**: `src/pages/HomePage.tsx` (628-700번 줄)
- **상세 문서**: `DECISIONS.md` → "2026-01-14 iOS Safari 무한 스와이프 뒤로가기" 섹션

### ✅ 사주 API 백엔드 직접 호출 (최종 해결, 2026-01-13)
...
### ✅ 썸네일 이미지 캐시 버스팅 (2026-01-13)
- **문제**: 썸네일 재생성 시 브라우저 캐시로 인해 이전 이미지 계속 표시
- **해결**: `imageCacheBuster` 상태로 URL에 버전 쿼리 파라미터 추가
- **핵심 파일**: `/components/MasterContentDetail.tsx`

### ✅ iOS 첫 번째 클릭 이벤트 누락 해결 (2026-01-12)
