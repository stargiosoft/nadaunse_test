...
export default function SajuDetail({ recordId, onClose }: SajuDetailProps) {
  ...
  const handleNextPage = () => {
    console.log('🔵 [SajuDetail] 다음 버튼 클릭됨');
    console.log('📌 [SajuDetail] 현재 페이지:', currentPage);
    console.log('📌 [SajuDetail] 전체 페이지:', totalPages);
    
    if (currentPage < totalPages) {
      console.log('➡️ [SajuDetail] 다음 페이지로 이동:', currentPage + 1);
      setCurrentPage(currentPage + 1);
    } else if (currentPage === totalPages) {
      // 마지막 페이지에서 "다음" 클릭 시 풀이 완료 페이지로 이동
      console.log('✅ [SajuDetail] 마지막 페이지 도달 → /result/complete로 이동');
      navigate('/result/complete');
    }
  };

  // 🔴 테스트용: 10페이지로 빠르게 이동
  const handleTestJumpToPage10 = () => {
    setCurrentPage(10);
  };
                  ...
                  <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
                    ...
                    <div className="content-stretch flex gap-[16px] items-center justify-end relative shrink-0">
                      ...
                      <button
                        ...
                        className="box-border content-stretch flex flex-col gap-[10px] h-[34px] items-center justify-center px-[8px] py-0 relative rounded-[12px] shrink-0 bg-transparent border-none cursor-pointer hover:bg-gray-100 transition-colors z-10"
                      >
                        <div className="content-stretch flex gap-[4px] items-center relative shrink-0 w-full pointer-events-none">
