...
export default function ProductDetail({ product, onBack, onPurchase }: ProductDetailProps) {
  ...
  useEffect(() => {
    const fetchCouponInfo = async () => {
      console.log('🎫 [ProductDetail] 쿠폰 정보 조회 시작');
      ...
      try {
        ...
        // 2. 로그인 상태: 보유 쿠폰 조회
        console.log('👤 [ProductDetail] 로그인 상태 → 쿠폰 조회');
        setIsLoggedIn(true);
        ...
        if (couponError) {
          console.error('❌ [ProductDetail] 쿠폰 조회 실패:', couponError);
          setCouponInfo(null);
          setIsLoadingCoupon(false);
          return;
        }

        console.log('✅ [ProductDetail] 쿠폰 조회 완료:', userCoupons);

        // 3. 쿠폰 없음
        if (!userCoupons || userCoupons.length === 0) {
          console.log('ℹ️ [ProductDetail] 보유 쿠폰 없음 → 혜택가 미노출');
          setCouponInfo(null);
          setIsLoadingCoupon(false);
          return;
        }
        ...
        // ⭐ 쿠폰 이름 디버깅
        console.log('🔍 [ProductDetail] 보유 쿠폰 이름 목록:', userCoupons.map((uc: any) => ({
          name: uc.coupons?.name,
          type: uc.coupons?.coupon_type,
          discount: uc.coupons?.discount_amount
        })));

        // 4. 가입축하쿠폰 보유 여부 확인 (coupon_type 또는 name으로 매칭)
        const welcomeCoupon = userCoupons.find((uc: any) => 
          uc.coupons?.coupon_type === 'welcome' || 
          uc.coupons?.name === '가입축하쿠폰'
        );
        const revisitCoupon = userCoupons.find((uc: any) => 
          uc.coupons?.coupon_type === 'revisit' || 
          uc.coupons?.name?.includes('재구매') ||
          uc.coupons?.name?.includes('재방문')
        );
        
        console.log('🎫 [ProductDetail] 쿠폰 매칭 결과:', {
          welcomeCoupon: welcomeCoupon ? `있음 (${welcomeCoupon.coupons?.name})` : '없음',
          revisitCoupon: revisitCoupon ? `있음 (${revisitCoupon.coupons?.name})` : '없음'
        });
        ...
        if (welcomeCoupon) {
          ...
          setCouponInfo({
            ...
            finalPrice,
            label: '재구매 혜택가',
            buttonText: `재구매 쿠폰 받고 ${finalPrice.toLocaleString()}원으로 풀이 보기`
          });
        } else {
          console.log('ℹ️ [ProductDetail] 다른 쿠폰 보유 (가입축하/재방문 아님) → 혜택가 미노출'); // ⭐ 로그 메시지 수정
          setCouponInfo(null);
        }

        setIsLoadingCoupon(false);
      } catch (error) {
        console.error('❌ [ProductDetail] 쿠폰 정보 조회 중 에러:', error);
        setCouponInfo(null);
        setIsLoadingCoupon(false);
      }
    };
  ...
  const handleCouponButtonClick = () => {
    if (!isLoggedIn) {
      // 로그아웃 상태 → 로그인 페이지로 이동 (결제 페이지 URL 저장)
      const paymentUrl = `/product/${product.id}/payment/new`;
      console.log('🔐 [ProductDetail] 로그인 페이지로 이동, 리다이렉트 URL 저장:', paymentUrl);
      localStorage.setItem('redirectAfterLogin', paymentUrl);
      navigate('/login/new');
    } else {
      // 로그인 상태 → 결제 페이지로 이동
      console.log('💳 [ProductDetail] 결제 페이지로 이동');
                ...
                <div className="box-border content-stretch flex flex-col items-start px-[16px] py-[8px] relative w-full">
                  <div className="content-stretch flex items-center overflow-clip relative shrink-0 w-full">
                    ...
                    >
                      <div className="flex flex-row items-center justify-center size-full">
                        <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[16px] py-[8px] relative w-full">
                          <p className={`${activeTab === 'description' ? "font-semibold text-[#151515]" : "font-medium text-[#999999]"} leading-[20px] not-italic relative shrink-0 text-[15px] text-nowrap tracking-[-0.45px] whitespace-pre`}>상품 설명</p>
                        </div>
                      </div>
                    </div>
                    {/* 풀이 원리 탭 */}
                    ...
                    >
                      <div className="flex flex-row items-center justify-center size-full">
                        <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[16px] py-[8px] relative w-full">
                          <p className={`${activeTab === 'principle' ? "font-semibold text-[#151515]" : "font-medium text-[#999999]"} leading-[20px] not-italic relative shrink-0 text-[15px] text-nowrap tracking-[-0.45px] whitespace-pre`}>풀이 원리</p>
                        </div>
                      </div>
                    </div>
                    {/* 맛보기 탭 */}
                    ...
                    >
                      <div className="flex flex-row items-center justify-center size-full">
                        <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[16px] py-[8px] relative w-full">
                          <p className={`${activeTab === 'preview' ? "font-semibold text-[#151515]" : "font-medium text-[#999999]"} leading-[20px] not-italic relative shrink-0 text-[15px] text-nowrap tracking-[-0.45px] whitespace-pre`}>맛보기</p>
                        </div>
                      </div>
                    </div>
                  </div>
                                ...
                                {!isLoadingCoupon && couponInfo && (
                                  <div className="content-stretch flex gap-[12px] items-center relative shrink-0 w-full">
                                    <p className="font-bold leading-[32.5px] not-italic relative shrink-0 text-[#48b2af] text-[22px] text-nowrap tracking-[-0.22px] whitespace-pre">
                                      {couponInfo.finalPrice.toLocaleString()}원
                                    </p>
                                ...
                                <div className="basis-0 content-stretch flex gap-[8px] grow items-center justify-center min-h-px min-w-px relative shrink-0">
                                  ...
                                  <div className="content-stretch flex gap-[4px] items-center relative shrink-0">
                                    <p className="font-medium leading-[22px] not-italic relative shrink-0 text-[0px] text-[14px] text-black text-nowrap tracking-[-0.42px] whitespace-pre">
                                      {couponInfo.hasWelcomeCoupon ? '첫 구매 쿠폰 받고' : '재구매 쿠폰 받고'}
                                      <span className="text-[#48b2af]"> </span>
                                      <span className="font-bold text-[#48b2af]">{couponInfo.finalPrice.toLocaleString()}원으로</span>
                                      <span>{` 풀이 보기`}</span>
                                    </p>
                                ...
                                <div className="bg-white relative rounded-[12px] shrink-0 w-full">
                                  <div className="flex flex-row items-center justify-center size-full">
                                    <div className="box-border content-stretch flex gap-[10px] items-center justify-center px-[12px] py-[10px] relative w-full">
                                      <p className="basis-0 font-normal grow leading-[23.5px] min-h-px min-w-px not-italic relative shrink-0 text-[#151515] text-[15px] tracking-[-0.3px]">그 사람과 끝까지 함께할 수 있을까요?</p>
                                    </div>
                                  </div>
                                </div>
                              </div>
                    ...
                    <div className="content-stretch flex flex-col gap-[4px] h-[298px] items-start justify-center relative shrink-0 w-[310px]">
                      <div className="content-stretch flex flex-col gap-[16px] items-start relative shrink-0 w-full">
                        <div className="content-stretch flex flex-col gap-[24px] items-center justify-center relative shrink-0 w-full">
                          ...
                          <p className="font-bold leading-[24px] min-w-full not-italic relative shrink-0 text-[18px] text-black text-center tracking-[-0.36px] w-[min-content]">우리 운세는 왜 다를까요?</p>
                        </div>
                        <div className="content-stretch flex flex-col gap-[4px] items-start relative shrink-0 w-full">
            ...
            <div className="content-stretch flex flex-col gap-[40px] items-center relative shrink-0 w-full">
                ...
                <div className="relative shrink-0 w-full">
                  <div className="flex flex-row items-center justify-center size-full">
                    <div className="box-border content-stretch flex gap-[10px] items-center justify-center pb-[20px] pt-[32px] px-[20px] relative w-full">
                      <p className="basis-0 font-semibold grow leading-[25.5px] min-h-px min-w-px not-italic relative shrink-0 text-[17px] text-black text-center tracking-[-0.34px]">아래는 일부 예시 해석입니다</p>
                    </div>
                  </div>
                </div>
                <div className="bg-[#f9f9f9] h-[12px] shrink-0 w-full" />
                ...
                <div className="h-[293px] relative shrink-0 w-full">
                  <div className="absolute content-stretch flex flex-col inset-0 items-start">
                    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full">
                      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
                        <div className="basis-0 content-stretch flex gap-[10px] grow items-center justify-center min-h-px min-w-px relative shrink-0">
                          <p className="basis-0 font-semibold grow leading-[28.5px] min-h-px min-w-px not-italic relative shrink-0 text-[#151515] text-[16px] tracking-[-0.32px]">01. 내 사랑, 오래 지속될 인연일까?</p>
                        </div>
                      </div>
                        ...
                        <div className="size-full">
                          <div className="box-border content-stretch flex gap-[10px] h-[252px] items-start px-[20px] py-[16px] relative w-full">
                            <div className="basis-0 content-stretch flex flex-col gap-[8px] grow items-start min-h-px min-w-px relative shrink-0">
                              ...
                              <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full">
                                <p className="font-normal leading-[23.5px] not-italic relative shrink-0 text-[15px] text-neutral-600 tracking-[-0.3px] w-full">이 만남은 처음부터 비교적 온화하고 안정적인 기운 속에서 시작되었습니다. 일주(日柱)의 기운과 상대방의 사주가 크게 충돌하지 않아, 첫 만남에서부터 서로에게 편안한 기류가 흐르고, 자연스럽게 가까워질 수 있는 끌림이 있었습니다. 당시의 관계는 불필요한 긴장감이나 갈등보다는 부드럽고 평온한 흐름으로 이어졌습니다. 겉으로 드러나는 모습만 보아도 불안정하거나 삐걱거림이 적었</p>
                              </div>
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="content-stretch flex gap-[8px] items-center justify-center relative shrink-0">
                      ...
                      <div className="content-stretch flex flex-col items-center justify-center relative shrink-0">
                        <div className="box-border content-stretch flex gap-[10px] items-center justify-center pb-0 pt-[3px] px-0 relative shrink-0">
                          <p className="font-medium leading-[22px] not-italic relative shrink-0 text-[#41a09e] text-[14px] text-nowrap tracking-[-0.42px]">여기까지만 공개돼요</p>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="h-[293px] relative shrink-0 w-full">
                  <div className="absolute content-stretch flex flex-col inset-0 items-start">
                    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full">
                      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
                        <div className="basis-0 content-stretch flex gap-[10px] grow items-center justify-center min-h-px min-w-px relative shrink-0">
                          <p className="basis-0 font-semibold grow leading-[28.5px] min-h-px min-w-px not-italic relative shrink-0 text-[#151515] text-[16px] tracking-[-0.32px]">02. 이 감정, 스쳐가는 바람일까 진짜일까?</p>
                        </div>
                      </div>
                      <div className="bg-[#f9f9f9] h-[252px] relative rounded-[12px] shrink-0 w-full">
                        <div className="size-full">
                    ...
                    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full">
                      <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
                        <div className="content-stretch flex gap-[10px] items-center justify-center relative shrink-0 w-full">
                          <p className="font-semibold leading-[28.5px] not-italic relative text-[#151515] text-[16px] tracking-[-0.32px] w-full">03. 우리 관계, 지금이 전환점일까?</p>
                        </div>
                      </div>
                              ...
                              <div className="content-stretch flex flex-col gap-[8px] items-start relative shrink-0 w-full">
                                ...
                                <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-full">
                                  <div className="content-stretch flex flex-col gap-[20px] items-center justify-center relative shrink-0 w-full">
                                    ...
                                    <p className="font-normal leading-[23.5px] min-w-full not-italic relative shrink-0 text-[15px] text-neutral-600 tracking-[-0.3px] w-[min-content]">이 카드는 호기심과 탐구심, 그리고 진실을 알고자 하는 열망을 상징합니다. 상대방을 향한 당신의 관심이 깊어지고 있으며, 마음속에 질문이 많아지는 시기일 수 있습니다. 다만, 모든 것을 성급히 판단하기보다 관찰하고 배워가야 할 때입니다. 말과 행동에서 솔직함이 중요하며, 작은 오해를 바로잡는 데 힘쓰면 관계가 훨씬 안정될 수 있습니다.</p>
                                  </div>
                                </div>
                              </div>
                            </div>
              ...
              <div className="px-[20px] w-full">
                <div className="bg-[#f0f8f8] box-border content-stretch flex gap-[10px] items-center justify-center px-[16px] py-[12px] relative rounded-[12px] shrink-0 w-full">
                  <div aria-hidden="true" className="absolute border border-[#7ed4d2] border-solid inset-0 pointer-events-none rounded-[12px]" />
                  <div className="basis-0 content-stretch flex flex-col gap-[8px] grow items-start min-h-px min-w-px relative shrink-0">
                    <p className="font-medium leading-[23.5px] not-italic relative shrink-0 text-[#41a09e] text-[15px] text-center tracking-[-0.3px] w-full">더 깊은 풀이는 구매 후 확인할 수 있습니다</p>
                  </div>
                </div>
              </div>
            </div>
