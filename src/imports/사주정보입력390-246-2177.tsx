import svgPaths from "./svg-pg6ite3akl";
import clsx from "clsx";

function Wrapper3({ children }: React.PropsWithChildren<{}>) {
  return (
    <div className="basis-0 content-stretch flex gap-[12px] grow items-center min-h-px min-w-px relative shrink-0">
      <p className="basis-0 font-['Pretendard_Variable:Regular',sans-serif] font-normal grow leading-[20px] min-h-px min-w-px relative shrink-0 text-[#b7b7b7] text-[15px] tracking-[-0.45px]">{children}</p>
    </div>
  );
}
...
function InputContainer4({ children }: React.PropsWithChildren<{}>) {
  return (
...
type InputFieldContainerTextProps = {
  text: string;
};

function InputFieldContainerText({ text }: InputFieldContainerTextProps) {
...
function InputContainer() {
  return (
    <InputContainer4>
      <InputFieldContainerText text="예: 홍길동" />
    </InputContainer4>
  );
}
...
function InputContainer1() {
  return (
    <InputContainer4>
      <InputFieldContainerText text="예: 1992-07-15 (양력)" />
    </InputContainer4>
  );
}
...
function InputFieldContainer() {
  return <Wrapper3>{`'-'하이픈 없이 숫자만 입력해 주세요`}</Wrapper3>;
}
...
export default function Component() {
  return (
    <div className="bg-white relative size-full" data-name="사주 정보 입력 _ 390">
