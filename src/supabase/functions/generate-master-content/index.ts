...
serve(async (req) => {
  ...
  try {
    ...
    // 4️⃣ 마스터 권한 확인 (users 테이블에서 is_master 체크)
    ...
    if (userError || !userData || !userData.is_master) {
      console.error('❌ 마스터 권한 없음:', user.id);
      return new Response(
    ...
    // 콘텐츠 정보 조회 (title, description, questioner_info, content_type)
    const { data: contentData, error: contentError } = await supabase
      .from('master_contents')
      .select('title, description, questioner_info, user_concern, content_type')
      .eq('id', contentId)
      .single()
